...
gSeg_repeated = function(n, l, edges, n0=0.05*n, n1=0.95*n, pval.appr=TRUE, skew.corr=TRUE, pval.perm=FALSE, B=100, alpha=1, kappa=1){
...
gcp1bynode = function(n, l, Ebynode, n0=ceiling(0.05*n), n1=floor(0.95*n), alpha=1, kappa=1){
  ...
  top = sum_Du_Duu-(2/n)*G_in*G_out
  bot = ((sum_Du2-(4*G_out^2)/n)*(sum_Duu2-(G_in^2)/n))^(1/2)
  covrho <- max(min(top/bot, 1), -1)
  ...
  Zod = (Ro1-Ro2 - (mu.Ro1-mu.Ro2))/sqrt(apply(cbind(V1 + V2 - 2*C12,rep(0,n)),1,max))
  Tod = abs(Zod)
  Zin_original = (Ri1-mu.Ri1)/sqrt(apply(cbind(V3,rep(0,n)),1,max))
  Tin = abs((Zin_original-covrho*Zod)/sqrt(1-covrho^2))
  
  Mout = apply(cbind(Tod,kappa*Zow),1,max)
  M = apply(cbind(Tin,alpha*Mout),1,max)
  tauhat = temp[which.max(M[n0:n1])]
  rmax = list(tauhat=tauhat, Zmax=M[tauhat], Zowmax=Zow[tauhat], Todmax=Tod[tauhat], Tinmax=Tin[tauhat], Zow=Zow, Tod=Tod, Tin=Tin, M=M, alpha=alpha, kappa=kappa)
  scanZ$rmax = rmax
...
## sub functions for p-value approximation
...
pval1 = function(n, l, edges, Ebynode, scanZ, skew.corr=TRUE, lower=ceiling(0.05*n), upper=floor(0.95*n)){
  ...
  if (skew.corr==FALSE){
    ...
    pvalM <- as.numeric(1-(1-min(pval.repeated1,1))*(1-min(pval.repeated2,1))*(1-min(pval.repeated3,1)))
    
    out.repeated = list(pval1 = pval1, pval2 = pval2, pval3 = pval3, pval = pvalM)
    output = out.repeated
  ...
  result.ur1 = pval1_sub_1(n,(br/alpha),r2,x2,lower,upper)        #Tod
  result.ur2 = pval1_sub_2(n,(br/(kappa*alpha)),r1,x1,lower,upper)  #Zow
  
  
  
  k = G_out+Duv-2*Du
  k_sim <- (k+t(k))/2  # G_out + Duv - Du -Dv
  iooa <- sum(Duv^2*Duu)
  ioob <- (sum(Duu*Du^2)-iooa + 2*sum(t(Duv*(Du-Duv))*Duu))
  iooc <- 2*(sum(Duv*Duu*(k_sim)))
  iood <- (sum(Duv^2)/2)*G_in - iooa
  iooe <- (sum(Du^2)-sum(Duv^2))*G_in - ioob
  ioof <- (sum(Duv*(G_out+Duv-2*Du))/2)*G_in - iooc
  
  iioa <- sum(t(Duv*Duu)*Duu) + sum(Duv*Duu^2)
  iiob <- 2*(sum(Duv*Duu)*G_in - iioa)
  iioc <- sum(Duu^2*(G_out-Du))
  iiod <- G_in^2*G_out - iioa - iiob - iioc
  
  
  # iio1: (R_in,1(t)^2 * R_out,1(t))
  # iio2: (R_in,1(t)^2 * R_out,2(t))
  iio1 <- iioa*t*(t-1)/(n*(n-1)) + (iiob+iioc)*t*(t-1)*(t-2)/(n*(n-1)*(n-2)) + iiod*t*(t-1)*(t-2)*(t-3)/(n*(n-1)*(n-2)*(n-3))
  iio2 <- iioc*(t*(n-t)*(n-t-1))/(n*(n-1)*(n-2)) + iiod*(t*(t-1)*(n-t)*(n-t-1))/(n*(n-1)*(n-2)*(n-3))
  
  # ioo1: E(R_in,1(t) * R_out,1(t)^2)
  # ioo2: E(R_in,1(t) * R_out,1(t) * R_out,2(t))
  # ioo3: E(R_in,1(t) * R_out,2(t)^2)
  ioo1 <- iooa*t*(t-1)/(n*(n-1)) + (ioob+iood)*t*(t-1)*(t-2)/(n*(n-1)*(n-2)) + (iooc+iooe)*t*(t-1)*(t-2)*(t-3)/(n*(n-1)*(n-2)*(n-3)) + ioof*t*(t-1)*(t-2)*(t-3)*(t-4)/(n*(n-1)*(n-2)*(n-3)*(n-4))
  ioo2 <- (1/2)*iooc*(t*(t-1)*(n-t)*(n-t-1))/(n*(n-1)*(n-2)*(n-3)) + ioof*(t*(t-1)*(t-2)*(n-t)*(n-t-1))/(n*(n-1)*(n-2)*(n-3)*(n-4))
  ioo3 <- iood*t*(n-t)*(n-t-1)/(n*(n-1)*(n-2)) + iooe*(t*(n-t)*(n-t-1)*(n-t-2))/(n*(n-1)*(n-2)*(n-3)) + ioof*(t*(n-t)*(n-t-1)*(n-t-2)*(n-t-3))/(n*(n-1)*(n-2)*(n-3)*(n-4))
  
  
  iiod <- iio1 - iio2               # E(R_in,1(t)^2 * R_out,d(t))
  iodod <- ioo1 + ioo3 - 2*ioo2     # E(R_in,1(t) * R_out,d(t)^2)
  
  sig_iiod <- V3 * sqrt(sig12)
  sig_iodod <- sig12 * sqrt(V3)
  
  E_iod <- C13 - C23 + E1*E3 - E2*E3  # E(R_in(t)*R_out,d(t)) = Cov(R_in(t), R_out,d(t)) + E(R_in(t))*E(R_out,d(t))
  E_ii <- V3 + E3^2                   # E(R_in(t)^2) = Var(R_in(t)) + E(R_in(t))^2
  E_odod <- sig12 + mu2^2
  
  
  ziiod <- (iiod - 2*E3*E_iod - mu2*E_ii + 2*(E3^2)*mu2)/sig_iiod        # E(Z_in(t)^2 * Z_out,d(t))
  ziodod <- (iodod - 2*mu2*E_iod - E3*E_odod + 2*E3*(mu2^2))/sig_iodod   # E(Z_in(t) * Z_out,d(t)^2)
  ...
  r_tilde <- (r3 - 3*covrho*ziiod + 3*(covrho^2)*ziodod - (covrho^3)*r2)/(1-(covrho^2))^(3/2)
  br3 = scanZ$rmax$Zmax
  result.ur_in = pval1_sub_1(n,br3,r_tilde,x3,lower,upper)
  ...
  output = list(pval1 = pval1, pval2 = pval2, pval3 = pval3, pval = pvalM)
...
permpval1 = function(n, l, Ebynode, scanZ, B, n0=ceiling(0.05*n), n1=floor(0.95*n), alpha=alpha, kappa=kappa){
  ...
  Z1.repeated = Z2.repeated = Z3.repeated = Zmax.repeated = matrix(0,B,n)
  for(b in 1:B){
    if(b%%1000 ==0) {
  ...
  maxZ3s = sort(maxZ3)
  maxM = apply(Zmax.repeated[,n0:n1],1,max)
  maxMs = sort(maxM)
  ...
  pvalM = length(which(maxMs>=scanZ$rmax$Zmax))/B
  
  output = list(pval1=pval1, pval2=pval2, pval3=pval3, pval=pvalM,
                maxZ1s=maxZ1s, maxZ2s=maxZ2s, maxZ3s=maxZ3s, Z1=Z1.repeated, Z2=Z2.repeated, Z3=Z3.repeated)
